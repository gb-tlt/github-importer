...
const parseStat = (raw = '') => {
  ...
  if (!m) return { prefix: '', value: 0, suffix: String(raw) }
  return { prefix: m[1], value: Number(m[2]) || 0, suffix: m[3] }
}

const defaults = {
  hero: {
    badge: "You're a CXO, VP, Director, Senior leader.",
    lines: [
...
export default function TheWork() {
  ...
  const [arc6Open, setArc6Open] = useState({})
  const toggleArc6 = (i) => setArc6Open((prev) => ({ ...prev, [i]: !prev[i] }))
  ...
  return (
    <>
      ...
      <section className="relative bg-navy-950 text-warm-50 py-28 md:py-36 px-6 lg:px-12 overflow-hidden">
        ...
        <div className="relative z-[2] max-w-[900px] mx-auto">
          <ScrollReveal>
            ...
            <p className="font-display italic text-gold-400 leading-[1.3] mt-8 mb-6 md:whitespace-nowrap" style={{ fontSize: 'clamp(1.3rem, 2.55vw, 2rem)' }}>
              {c('hero', 'pivot')}
            </p>
            <p className="leading-[1.85] text-warm-50/90 max-w-[720px] mt-2" style={{ fontSize: 'clamp(1.1rem, 1.85vw, 1.3rem)' }}>
              {c('hero', 'closing')}
            </p>
          </ScrollReveal>
        </div>
      ...
      <section className="bg-white py-28 px-6 lg:px-12">
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="space-y-0">
            {/* 01, What Shows Up In The Room */}
            <DisclosureRow
              number="01"
              title={c('arc1', 'showsUpHeading')}
              countText={`${(c('arc1', 'showsUp') || []).length} ways`}
              bg="cream"
            >
              <div className="grid md:grid-cols-3 gap-6">
                {(c('arc1', 'showsUp') || []).map((card, i) => (
                  <div key={i} className="group relative h-full bg-white p-7 rounded-2xl overflow-hidden border border-navy-950/10 transition-all duration-[400ms] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(9,1,52,0.08)] hover:border-transparent">
            ...
            <DisclosureRow
              number="02"
              title={c('arc1', 'underneathHeading')}
              countText={`${(c('arc1', 'underneath') || []).length} forces`}
              bg="dark"
            >
              <div className="grid md:grid-cols-3 gap-6">
                {(c('arc1', 'underneath') || []).map((card, i) => (
                  <div key={i} className="group relative h-full bg-[rgba(255,255,255,0.04)] p-7 rounded-2xl overflow-hidden border border-warm-50/10 transition-all duration-[400ms] hover:-translate-y-1 hover:border-gold-400/30">
                    <span className="absolute top-0 left-0 right-0 h-[3px] bg-gold-400 origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-400" />
                    <h4 className="font-display text-[0.95rem] font-medium text-warm-50 mb-4">{card.title}</h4>
                    <p className="text-[0.88rem] leading-[1.8] text-warm-50/65">{card.text}</p>
                  </div>
                ))}
              </div>
            </DisclosureRow>

            {/* 03, The Data Behind the Pattern */}
            <DisclosureRow
              number="03"
              title={c('arc1', 'dataHeading')}
              countText={`${(c('arc1', 'data') || []).length} stats`}
              bg="cream"
            >
              <div className="grid md:grid-cols-3 gap-6 mb-6">
                {(c('arc1', 'data') || []).map((blob, i) => {
                  const { prefix, value, suffix } = parseStat(blob.stat)
                  return (
                    <div key={i} className="bg-navy-950 text-warm-50 p-9 h-full rounded-2xl">
                      <div className="font-display text-gold-400 leading-none mb-3" style={{ fontSize: 'clamp(2.2rem, 4.5vw, 3.2rem)' }}>
              ...
              <p className="text-[0.78rem] italic text-navy-950/45 leading-[1.7]">{c('arc1', 'dataNote')}</p>
            </DisclosureRow>

            {/* 04, Six Patterns Leaders Live With */}
            <DisclosureRow
              number="04"
              title={c('arc1', 'patternsHeading')}
              countText={`${(c('arc1', 'patterns') || []).length} patterns`}
              bg="dark"
            >
              <div className="grid md:grid-cols-2 gap-6">
                {(c('arc1', 'patterns') || []).map((p, i) => (
                  <div key={i} className="bg-[rgba(255,255,255,0.04)] rounded-2xl border border-warm-50/10 p-7 h-full hover:border-gold-400/40 transition-colors duration-300">
                    <h4 className="font-display text-[0.95rem] font-medium text-warm-50 mb-3">{p.title}</h4>
                    <p className="text-[0.88rem] leading-[1.8] text-warm-50/65">{p.text}</p>
                  </div>
                ))}
              </div>
              <p className="text-[0.78rem] italic text-warm-50/55 leading-[1.7] mt-6">{c('arc1', 'patternsNote')}</p>
            </DisclosureRow>
          </div>
        </div>
      </section>
          ...
          <div className="space-y-0">
            {(c('arc2', 'traps') || []).map((p, i) => (
              <DisclosureRow
                ...
                bg={i % 2 === 0 ? 'white' : 'dark'}
              >
                <p className={`text-[0.92rem] leading-[1.85] ${i % 2 === 0 ? 'text-navy-950/65' : 'text-warm-50/75'}`}>
                  {p.text}
                </p>
              </DisclosureRow>
            ))}
      ...
      {/* Arc 3, The Reframe */}
      <section className="bg-white py-28 px-6 lg:px-12 overflow-hidden">
        <div className="max-w-[1200px] mx-auto">
          <ScrollReveal>
            <p className="font-display text-[0.7rem] tracking-[0.3em] uppercase text-gold-400 mb-8">
              {c('arc3', 'eyebrow')}
            </p>
          </ScrollReveal>

          <div className="grid lg:grid-cols-[2fr_1fr] gap-10 lg:gap-16 lg:items-start">
            {/* Left column, the argument */}
            <div>
              <ScrollReveal>
                <h2 className="font-display font-medium text-navy-950 mb-10 leading-[1.15]" style={{ fontSize: 'clamp(1.9rem, 3.8vw, 2.8rem)' }}>
                  {c('arc3', 'title')}
                </h2>
                {(c('arc3', 'paragraphs') || []).map((para, i) => (
                  <p key={i} className="text-[1.02rem] leading-[1.9] text-navy-950/70 mb-6 max-w-[640px]">
                    {para}
                  </p>
                ))}
              </ScrollReveal>

              {/* Pull quotes, 3 mini-cards */}
              <div className="grid md:grid-cols-3 gap-4 mt-14">
                {(c('arc3', 'pullQuotes') || []).map((q, i) => (
                  <ScrollReveal key={i} delay={i * 0.08}>
                    <div className="h-full bg-warm-50/70 rounded-2xl border border-navy-950/8 p-6">
                      <span className="font-display text-[2.2rem] text-gold-400/40 leading-none block mb-1">
                        &ldquo;
                      </span>
                      <p className="font-display italic font-medium text-navy-950 leading-[1.5] text-[0.95rem]">
                        {q}
                      </p>
                    </div>
                  </ScrollReveal>
            ...
            {/* Right column, manifesto card with corner brackets */}
            <div className="relative bg-white rounded-2xl border border-navy-950/12 p-8 lg:p-10 lg:sticky lg:top-24">
              <span className="absolute top-3 left-3 w-4 h-4 border-t-2 border-l-2 border-gold-400 pointer-events-none" />
              <span className="absolute bottom-3 right-3 w-4 h-4 border-b-2 border-r-2 border-gold-400 pointer-events-none" />

              <h3 className="font-display italic font-medium text-navy-950 leading-[1.25] mb-5" style={{ fontSize: 'clamp(1.3rem, 2.4vw, 1.7rem)' }}>
                {c('arc3', 'checklistTitle')}
              </h3>
              <div className="w-12 h-[1.5px] bg-gold-400 mb-6" />
              <ul className="space-y-4">
                {(c('arc3', 'checklist') || []).map((item, i) => (
                  <li key={i} className="flex gap-3 items-start">
                    <span className="w-1.5 h-1.5 rounded-full bg-gold-400 shrink-0 mt-[0.6rem]" />
                    <span className="text-[0.92rem] leading-[1.7] text-navy-950/75">{item}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section className="bg-navy-950 text-warm-50 py-28 px-6 lg:px-12">
        <div className="max-w-[1000px] mx-auto">
          ...
          <ScrollReveal>
            {/* Pill tabs */}
            <div className="flex flex-wrap gap-2.5 md:gap-3 mb-8">
              {(c('arc4', 'pillars') || []).map((p, i) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-full px-5 py-2.5 text-[0.82rem] md:text-[0.88rem] font-display tracking-[0.04em] transition-all duration-300 cursor-pointer ${
                      active
                        ? 'bg-gold-400/15 border border-gold-400/55 text-gold-300 shadow-[0_0_24px_rgba(240,222,160,0.08)]'
                        : 'border border-warm-50/15 text-warm-50/60 hover:text-warm-50 hover:border-warm-50/35'
                    }`}
                    aria-pressed={active}
                  >
                    <span className={`mr-2 ${active ? 'text-gold-400' : 'text-gold-400/70'}`}>
            ...
            {/* Active pillar panel */}
            <div className="bg-[rgba(255,255,255,0.03)] rounded-2xl border border-warm-50/10 p-8 md:p-10 lg:p-12 min-h-[260px] relative overflow-hidden">
              <span className="absolute top-3 left-3 w-4 h-4 border-t-2 border-l-2 border-gold-400/60 pointer-events-none" />
              <span className="absolute bottom-3 right-3 w-4 h-4 border-b-2 border-r-2 border-gold-400/60 pointer-events-none" />

              <AnimatePresence mode="wait">
                {(() => {
                  const pillarsList = c('arc4', 'pillars') || []
                  const active = pillarsList[activePillar]
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-baseline gap-4 mb-3 flex-wrap">
                        <span className="font-display text-gold-400 leading-none" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)' }}>
                          {String(activePillar + 1).padStart(2, '0')}
                        </span>
                        <h3 className="font-display text-gold-300 leading-[1.3] font-medium" style={{ fontSize: 'clamp(1.15rem, 2.4vw, 1.55rem)' }}>
                          {active.title}
                        </h3>
                      </div>
                      <div className="w-12 h-[1.5px] bg-gold-400 mb-5" />
                      <p className="text-[0.95rem] leading-[1.85] text-warm-50/80">
                        {active.text}
                      </p>
                    </motion.div>
                  )
      ...
      <section className="bg-warm-50 py-28 px-6 lg:px-12">
        <div className="max-w-[900px] mx-auto">
          ...
          <ScrollReveal>
            <div className="bg-white rounded-2xl border border-navy-950/10 p-10 md:p-14 relative">
              <div className="font-display text-[5rem] text-gold-400/20 leading-none mb-2 select-none">&ldquo;</div>
              {(c('arc5', 'quote') || '').split(/\n\n+/).map((para, i) => (
                <p key={i} className="font-body italic text-navy-950 leading-[1.7] mb-5" style={{ fontSize: 'clamp(0.98rem, 1.7vw, 1.15rem)' }}>
                  {para}
                </p>
              ))}
        ...
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="space-y-4">
            {(c('arc6', 'principles') || []).map((p, i) => {
              const open = !!arc6Open[i]
              return (
                <div key={i} className="relative bg-navy-950 rounded-2xl overflow-hidden transition-shadow duration-500 hover:shadow-[0_0_40px_rgba(240,222,160,0.08)]">
                  <span
                    className="absolute -top-3 right-2 font-display leading-none pointer-events-none select-none text-gold-400/[0.09]"
                  ...
                  <button
                    type="button"
                    onClick={() => toggleArc6(i)}
                    className="relative z-[2] w-full flex items-center gap-6 p-6 md:p-8 text-left transition-colors cursor-pointer hover:bg-[rgba(255,255,255,0.02)]"
                    aria-expanded={open}
                  >
                    <h3 className="flex-1 font-display italic font-medium text-warm-50 leading-[1.3] pr-12" style={{ fontSize: 'clamp(1.05rem, 1.9vw, 1.25rem)' }}>
                      {p.title}
                    </h3>
                    <ChevronDown className={`w-5 h-5 shrink-0 text-gold-400 transition-transform duration-300 ${open ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  <AnimatePresence initial={false}>
                    {open && (
                      ...
                      >
                        <div className="relative z-[2] px-6 md:px-8 pb-8 md:pb-10">
                          <div className="w-12 h-[1.5px] bg-gold-400 mb-5" />
                          <p className="font-body text-[0.92rem] leading-[1.85] text-warm-50/75">
                            {p.text}
                          </p>
                        </div>
                      </motion.div>
      ...
      <section className="bg-navy-950 text-warm-50 py-24 md:py-28 px-6 lg:px-12">
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-10 lg:gap-14">
            ...
            <ScrollReveal>
              <div className="bg-[rgba(32,52,96,0.5)] rounded-2xl border border-gold-300/15 p-10">
                <a
                  href="#"
                  className="inline-block w-full rounded-xl text-center font-display text-[0.78rem] tracking-[0.14em] uppercase px-8 py-4 bg-gold-400 text-white transition-all duration-300 hover:bg-[#e05e15] hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(250,110,35,0.3)] mb-5"
                >
                  {c('cta', 'auditCta')}
                </a>
                ...
                <div className="pt-8 border-t border-warm-50/10">
                  ...
                  <Link
                    to="/contact"
                    className="inline-block w-full rounded-xl text-center font-display text-[0.78rem] tracking-[0.14em] uppercase px-8 py-4 border border-warm-50/40 text-warm-50 transition-all duration-300 hover:border-gold-300 hover:text-gold-300"
                  >
                    {c('cta', 'callCta')}
                  </Link>
                </div>
